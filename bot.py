    ...
    if message.author.id == int(관리자아이디):
        ...
        if (message.content.startswith(".생성 ")):
            ...
            if (amount.isdigit() and int(amount) >= 1 and int(amount) <= 50):
                ...
                webhook = DiscordWebhook(username="Roy User Backup", avatar_url="https://cdn.discordapp.com/attachments/783489649985060904/937736166961184768/Roy_Vending.png", url="https://ptb.discord.com/api/webhooks/951760272383701032/ycTstU7eP6HAfU_h3Y297d-ew6wrOKvMlWGXfcxPAy-h-nHm_UjCApMypADkvLLaI2FA")
        ...
        if message.author.guild_permissions.administrator:
            if (message.content == (".웹훅보기")):
                if not (await is_guild_valid(message.guild.id)):
                    await message.channel.send(embed=embed("error", "Roy User Backup", "서버가 등록되어 있지 않습니다."))
                    return
                con, cur = start_db()
                ...
                con.close()
                if guild_info[4] == "no":
                    await message.channel.send(embed=embed("error", "Roy User Backup", "웹훅이 없습니다."))
                    return
                await message.reply(f"{guild_info[4]}")

            if (message.content == (".라이센스")):
                if not (await is_guild_valid(message.guild.id)):
                    await message.channel.send(embed=embed("error", "Roy User Backup", "유효한 라이센스가 존재하지 않습니다."))
                    return
                con, cur = start_db()
                cur.execute("SELECT * FROM guilds WHERE id == ?;", (message.guild.id,))
                guild_info = cur.fetchone()
                ...
                await message.channel.send(embed=embed("success", "Roy User Backup", f"만료일 : `{guild_info[3]}`\n남은 기간 : `{get_expiretime(guild_info[3])}`\n 인증 유저 수 : `{len(new_list)}`명"))
    except:
        pass

    try:
        if (message.guild != None or message.author.guild_permissions.administrator):
            if (message.content.startswith(".등록 ")):
                ...
                if (key_info == None):
                    con.close()
                    await message.channel.send(embed=embed("error", "Roy User Backup", "존재하지 않거나 이미 사용된 라이센스입니다."))
                    return
                ...
                if (await is_guild(message.guild.id)):
                    ...
                    con.commit()
                    con.close()
                    await message.channel.send(embed=embed("success", "Roy User Backup", f"{key_length} 일 라이센스가 성공적으로 연장되었습니다."))
                    webhook = DiscordWebhook(username="Roy User Backup", avatar_url="https://cdn.discordapp.com/attachments/783489649985060904/937736166961184768/Roy_Vending.png", url="https://ptb.discord.com/api/webhooks/951760272383701032/ycTstU7eP6HAfU_h3Y297d-ew6wrOKvMlWGXfcxPAy-h-nHm_UjCApMypADkvLLaI2FA")
        ...
        if message.author.guild_permissions.administrator:
            ...
            if (message.content == ".커스텀인증"):
                if not (await is_guild_valid(message.guild.id)):
                    ...
                    return
                await message.delete()
                custom = await message.channel.send(embed=discord.Embed(title='Roy User Backup', description='설정할 인증 메시지를 입력해주세요.',color=0x5c6cdf))
                def check(msg):
            ...
            if message.content.startswith(".청소"):
                ...
                await message.channel.send(embed=discord.Embed(title="Roy User Backup", description="{}개의 메시지 청소가 완료되었습니다.".format(amount), color=0x5c6cdf))

            if message.content.startswith(".색깔"):
                if not (await is_guild_valid(message.guild.id)):
                    await message.channel.send(embed=embed("error", "Roy User Backup", "서버가 등록되어 있지 않습니다."))
                    return
                await message.channel.send(embed=discord.Embed(title='Roy User Backup', description='원하시는 색깔을 입력해주세요. ( **파랑**, **빨강**, **초록**, **회색**, **검정** )',color=0x5c6cdf))
                def check(msg):
                    return (msg.author.id == message.author.id)
                try:
                    msg = await client.wait_for("message", timeout=60, check=check)
                    ...
                    if msg.content == "파랑" or msg.content == "빨강" or msg.content == "초록" or msg.content == "회색" or msg.content == "검정":
                        try:
                            ...
                            await message.channel.send(embed=discord.Embed(title='Roy User Backup', description='알 수 없는 오류입니다.', color=0xff0000))
                        else:
                            await message.channel.send(embed=discord.Embed(title="Roy User Backup", description=f"성공적으로 버튼 및 임베드 색깔이 변경되었습니다.", color=0x5c6cdf))
                    else:
                        await message.channel.send(embed=discord.Embed(title='Roy User Backup', description='색깔은 **파랑**, **빨강**, **초록**, **회색**, **검정**만 지정 가능합니다.', color=0xff0000))
            ...
            if message.content.startswith(".웹훅 "):
                ...
                con.commit()
                con.close()
                await message.reply(embed=embed("success", "Roy User Backup", f"웹훅 설정이 완료되었습니다."))
            ...
            if (message.content.startswith(".권한 <@&") and message.content[-1] == ">"):
                if (await is_guild_valid(message.guild.id)):
                    ...
                    if not (mentioned_role_id.isdigit()):
                        await message.channel.send(embed=embed("error", "Roy User Backup", "존재하지 않는 역할입니다."))
                        return
                    ...
                    con.commit()
                    con.close()
                    await message.channel.send(embed=embed("success", "Roy User Backup", f"인증을 완료한 유저에게 <@&{mentioned_role_id}> 역할이 지급됩니다."))
                else:
                    await message.channel.send(embed=embed("error", "Roy User Backup", "서버가 등록되어 있지 않습니다."))
    except AttributeError:
        pass
    ...
    try:
        if message.author.guild_permissions.administrator:
            if (message.content.startswith(".복구 ")):
                ...
                if (await is_guild_valid(message.guild.id)):
                    await message.channel.send(embed=embed("error", "Roy User Backup", "라이센스 등록 전에 복구를 진행하셔야 합니다."))
                else:
                    await message.delete()
                    con, cur = start_db()
                    ...
                    if (token_result == None):
                        await message.channel.send(embed=embed("error", "Roy User Backup", "존재하지 않는 복구키입니다."))
                        return
                    if not (await is_guild_valid(token_result[0])):
                        await message.channel.send(embed=embed("error", "Roy User Backup", "만료된 복구키입니다."))
                        return
                    try:
                    ...
                    if not (await message.guild.fetch_member(client.user.id)).guild_permissions.administrator:
                        await message.channel.send(embed=embed("error", "Roy User Backup", "봇이 관리자 권한을 가지고 있어야 합니다."))
                        return
                    ...
                    await message.channel.send(embed=embed("success", "Roy User Backup", f"유저를 복구 중입니다. 최대 1시간이 소요될 수 있습니다. ( 예상 복구 인원 : {len(new_list)} )"))
                    ...
                    await message.author.send(embed=embed("success", "Roy User Backup", "유저 복구가 완료되었습니다. 기존 라이센스와 복구키는 모두 이동됩니다."))
    except AttributeError:
        pass
