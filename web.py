...
async def callback():
    ...
    if not (str(state).isdigit()):
        print('1')
        return render_template("error.html", title="인증 실패", dese="터졌거나 존재하지 않는 인증 링크입니다.")
    ...
    try:
        ...
        if user == None:
            print('7')
            return render_template("error.html", title="인증 실패", dese="서버에 입장해 있지 않는 유저입니다.")
        ...
        try:
            ...
            return render_template("ok.html", title="인증 성공", nickname=f"닉네임 : {user.name}#{user.discriminator}", server=f"서버 이름 : {guild.name}", role=f"역할 정보 : {role.name}")


if __name__ == "__main__":
    try:
        app.run(host='0.0.0.0', port=440)
    except Exception as e:
        print(e)
    else:
